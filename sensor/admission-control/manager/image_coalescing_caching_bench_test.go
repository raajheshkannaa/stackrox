...
func benchSettings() *sensor.AdmissionControlSettings {
...
func setupManager(b *testing.B, imgClient sensor.ImageServiceClient) *manager {
	...
	mgr.ProcessNewSettings(benchSettings())
	mgr.Start()
...
func BenchmarkAdmissionReview_BaselineColdCache(b *testing.B) {
	...
	for b.Loop() {
		...
		mgr.imageNameToImageCacheKey.Access(func(m *map[string]string) { *m = make(map[string]string) })
