...
// TestGetNamespaceLabels_MultipleNamespacesError tests the error case when multiple namespaces
// are found with the same cluster ID and name (data integrity violation)
func (s *namespaceDatastoreComprehensiveSuite) TestGetNamespaceLabels_MultipleNamespacesError() {
	ctx := s.testContexts[testutils.UnrestrictedReadWriteCtx]

	// Create two namespaces with the same cluster ID and name but different IDs
	// This simulates a data integrity violation
	clusterID := testconsts.Cluster1
	namespaceName := "duplicate-namespace"

	ns1 := fixtures.GetScopedNamespace(uuid.NewV4().String(), clusterID, namespaceName)
	ns1.Labels = map[string]string{"instance": "first"}
	err := s.datastore.AddNamespace(ctx, ns1)
	s.Require().NoError(err)
	s.testNamespaceIDs = append(s.testNamespaceIDs, ns1.GetId())

	ns2 := fixtures.GetScopedNamespace(uuid.NewV4().String(), clusterID, namespaceName)
	ns2.Labels = map[string]string{"instance": "second"}
	err = s.datastore.AddNamespace(ctx, ns2)
	s.Require().NoError(err)
	s.testNamespaceIDs = append(s.testNamespaceIDs, ns2.GetId())

	// Call GetNamespaceLabels - should return an error
	labels, err := s.datastore.GetNamespaceLabels(ctx, clusterID, namespaceName)

	// Verify invariant violation error is returned
	s.Require().Error(err)
	s.Require().Nil(labels)
	s.Require().True(errors.Is(err, errox.InvariantViolation), "expected invariant violation error, got: %v", err)
}
